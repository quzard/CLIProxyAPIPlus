...
func normalizeResponseSubsequentRequest(rawJSON []byte, lastRequest []byte, lastResponseOutput []byte, allowIncrementalInputWithPreviousResponseID bool) ([]byte, []byte, *interfaces.ErrorMessage) {
	...
	if shouldReplaceWebsocketTranscript(rawJSON, nextInput) {
		...
		normalizedInput := gjson.GetBytes(normalized, "input")
		if augmented := ensureFunctionCallOutputs(lastResponseOutput, normalizedInput.Raw); augmented != normalizedInput.Raw {
			normalized, _ = sjson.SetRawBytes(normalized, "input", []byte(augmented))
		}
		return normalized, bytes.Clone(normalized), nil
	}
	...
	if allowIncrementalInputWithPreviousResponseID {
		if prev := strings.TrimSpace(gjson.GetBytes(rawJSON, "previous_response_id").String()); prev != "" {
			...
			// function_call_output in incremental input. When tool execution is
			// interrupted (e.g. Ctrl+C), the client may omit these and upstream
			// rejects the follow-up payload.
	...
	if augmented := ensureFunctionCallOutputs(lastResponseOutput, mergedInput); augmented != mergedInput {
		mergedInput = augmented
	}
